...
router.get("/:id", async (req, res) => {
    ...
    try {
        ...
        const { data: usuario, error } = await supabase
            .from("users")
            .select("id, nombre, rol, avatar_url, banner_url, descripcion")
            .eq("id", id)
            .maybeSingle();
...
/* ==========================
   ...
   Subir/cambiar tu banner de perfil
   (el cliente ya lo manda recortado, ej. 960x300)
========================== */

router.post("/me/banner", verifyToken, upload.single("banner"), async (req, res) => {
    ...
    try {
        ...
        const nombreArchivo = `${req.userId}-banner.${extension}`;
